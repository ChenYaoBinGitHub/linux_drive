...
static int __init hello_init(void){
    ...
    if (ret < 0) {
      printk(KERN_CRIT "231 HelloModual can't register major number.\n");
      return ret;
    }
    printk(KERN_ALERT "HelloModual initialized.\n");
    return 0;
}

static void __exit hello_exit(void){
    unregister_chrdev(HELLO_MAJOR, DEVICE_NAME);
    printk("HelloModual removed.\n");
}
